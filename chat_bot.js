...
client.on('message',async (channel, userstate, message, self) => {
  ...
  if (message == 'VoHiYo') { //回復 vohiyo 
    client.say(channel, `VoHiYo VoHiYo ${userstate['display-name']} VoHiYo VoHiYo `);
  }


  if (message.startsWith('!')) { //如果訊息前面有 !
    let command, arr_message;
    const new_message = message.slice(1); // 去掉第一個字 "!"
    if (new_message.includes(" ")) {  //如果裡面有" "
    ...
    if (response) { //如果有指令則回覆，沒有則會是null
